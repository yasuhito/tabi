class User < ActiveRecord::Base
  ...
  attr_accessible :email, :remember_me, :fb_user_id, :name, :image
  ...
  def self.find_or_create_from_auth_hash(auth_hash)
    ...
    if user = User.where(:fb_user_id => user_data.id).first
      ...
      User.create!(:fb_user_id => user_data.id, :name => user_data.name, :email => user_data.email, :image => auth_hash.info.image)
    end
  end
end
